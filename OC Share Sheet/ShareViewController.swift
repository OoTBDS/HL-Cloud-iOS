...
//            nameFolder =  nameFolder.substringWithRange(Range<String.Index>(start:nameFolder.startIndex, end: advance(nameFolder.startIndex, 20)))
             ...
             nameFolder =  nameFolder.substringWithRange(Range<String.Index>(start:nameFolder.startIndex, end: nameFolder.startIndex.advancedBy(20)))
    ...
    func sendTheFilesToOwnCloud() {
        ...
        if user != nil {
            ...
            //mv
//           for (index, url : NSURL) in (self.filesSelected.enumerate()){
            for (index, url) in self.filesSelected.enumerate(){
...
//                    let fileLength = NSFileManager.defaultManager().attributesOfItemAtPath(url.path!, error: nil)![NSFileSize] as! Int
                    ...
                    do{
                         fileLength = try NSFileManager.defaultManager().attributesOfItemAtPath(url.path!)[NSFileSize] as! Int
                    }catch _{
...
//                    for (index, current) in (enumerate(attachments)){
                        ...
                        if current.hasItemConformingToTypeIdentifier(kUTTypeItem as String){
                            ...
                            //mv
                            current.loadItemForTypeIdentifier(kUTTypeItem as String, options: nil, completionHandler: { (NSSecureCoding, error: NSError!) in
                                //
//                            })
//                            
...
//        let fileSizeInBytes = NSFileManager.defaultManager().attributesOfItemAtPath(url.path!, error: nil)![NSFileSize] as? Double
        ...
        var fileSizeInBytes: Double!
        do{
            fileSizeInBytes = try NSFileManager.defaultManager().attributesOfItemAtPath(url.path!)[NSFileSize] as? Double
        }catch _{
